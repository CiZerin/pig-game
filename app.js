...
*/
var scores, roundScore, activePlayer, gamePlaying, finalScore, diceDom;
...
document.querySelector('.btn-roll').addEventListener('click', function() {
  if (gamePlaying) {
    ...
    //3 Update the round score but not if roll was 1
    if ((dice[0] !== 1 && dice[1] !== 1) && dice[0] + dice[1] !== 12) {
      //add score
      roundScore += dice[0]+dice[1];
      document.querySelector('#current-'+activePlayer).textContent = roundScore;
    }
    else if (dice[0] + dice[1] === 12) {
...
document.querySelector('.btn-hold').addEventListener('click', function() {
  if (gamePlaying) {
    ...
    finalScore = document.querySelector('.final-score').value*1;
